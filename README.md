...
The host Pi process keeps the TUI, session, and provider credentials. File tools, bash, and `!` commands run in a Gondolin micro-VM against a disposable standalone clone. Use `/sandbox status`, `/sandbox tools`, `/sandbox diff`, `/sandbox apply`, or `/sandbox destroy`.
